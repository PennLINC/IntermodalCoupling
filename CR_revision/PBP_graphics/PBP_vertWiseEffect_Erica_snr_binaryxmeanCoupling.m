...
%custommap=colormap('plasma')
%newmap = plasma;                    %starting map
%ncol = size(newmap,1);           %how big is it?
%zpos = 1 + floor(1/5 * ncol);    %2/3 of way through
%newmap(zpos,:) = [0.75 0.75 0.75];        %set that position to white
%custommap=colormap(newmap);   
