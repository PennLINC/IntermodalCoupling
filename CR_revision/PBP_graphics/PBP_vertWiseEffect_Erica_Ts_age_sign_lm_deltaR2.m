...
%% calculated by the conversion - 1 point on the numeric scale = 7 on the color scale: range values -6 to 30, color scale 256 spots. Lowest value 2.8 (~19 colorblock values)
%custommap([1:25],:)=repmat([.84 .03 0.94],25,1);
%custommap([26:63],:)=repmat([0.75 0.75 0.75],38,1);
custommap([29:35],:)=repmat([0.75 0.75 0.75],7,1);
%custommap(57,:)=[0.75 0.75 0.75];
%custommap([1:43],:)=repmat([.84 .03 0.94],43,1);
...
print('-dpng','-r600',['/project/imco/baller/results/CR_revision/images/pbp/' char(name)])
